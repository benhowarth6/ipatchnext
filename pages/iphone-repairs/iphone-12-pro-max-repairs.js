...
const { id, name, description, price } = repairs;
...
const product = {
  ...
  imageSrc: '/iphone/b/12-pro-max.jpg',
...
const repairCount = { totalCount: 9 }
...
export default function Example() {
  ...
  const cartItems = Object.keys(cart.repairs).map(key => {
    const repair = repairs.find(({ id }) => `${id}` === `${key}`);
    return {
      ...cart.repairs[key],
      pricePerUnit: repairs.price
    }
  });
  ...
  function addToCart({ id }) {
    updateCart((prev) => {
      ...
      if (cart.repairs[id]) {
        cart.repairs[id].quantity = cart.repairs[id].quantity + 1;
      } else {
        cart.repairs[id] = {
          id,
          quantity: 1
        ...
        <div className="max-w-2xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8 lg:grid lg:grid-cols-2 lg:gap-x-8">
          ...
          <div className="mt-10 lg:max-w-lg lg:col-start-1 lg:row-start-2 lg:self-start">
            <section aria-labelledby="options-heading">
              ...
              <form>
              {repairs.map(repair => {
                        const { id, name, description, price } = repair;
                        return (
                          <div>
                ...
                <div className="mt-10">
                  <button onClick={() => addToCart({ id })} className="w-full bg-blue-600 border border-transparent rounded-md py-3 px-8 flex items-center justify-center text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-50 focus:ring-blue-500">
                    Book Repair
                  </button>
                </div>
