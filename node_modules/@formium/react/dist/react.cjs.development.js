...
var Yup = require('yup');
var _eval = require('@formium/eval');
...
var useIsomorphicLayoutEffect = /*#__PURE__*/utils.canUseDOM() ? React.useLayoutEffect : React.useEffect;
...
var makeResolver = function makeResolver(values) {
  return function (path) {
    return formik.getIn(values, path);
  };
};

var shouldExecuteAction = function shouldExecuteAction(valuesMap, touched, element, condition) {
  if (element.type === types.FormElementType.RADIO || element.type === types.FormElementType.CHECKBOX) {
    return _eval.evaluateLogic(makeResolver(valuesMap), condition);
  } else if (formik.getIn(touched, element.slug)) {
    return _eval.evaluateLogic(makeResolver(valuesMap), condition);
  }
...
function getChoiceMap(fields) {
  ...
  Object.values(fields).filter(function (i) {
    return i.type === types.FormElementType.CHOICE;
...
var unstable_FormiumLogic = /*#__PURE__*/React.memo(function (_ref) {
  var form = _ref.form,
  ...
  var valuesMap = React.useMemo(function () {
    return Object.values(fields || {}).reduce(function (prev, curr) {
      if (curr.type === types.FormElementType.CHOICE) {
        prev[curr.slug] = curr.title;
      } else if (curr.type !== types.FormElementType.PAGE && curr.type !== types.FormElementType.GROUP) {
  ...
  React.useEffect(function () {
    if (element.actions && element.actions.length > 0 && element.actions.forEach) {
      element.actions.forEach(function (item) {
        // Field-level actions
        if (item.details.to.type === types.FormElementActionDetailsToType.FIELD) {
          // hide
          if (item.action === types.FormElementActionAction.HIDE) {
          ...
          if (item.action === types.FormElementActionAction.SHOW) {
            setFieldProperty(item.details.to.value, {
...
function Header(_ref) {
  ...
  return React.createElement(React.Fragment, null, page.title ? pageIndex === 0 ? React.createElement("h1", null, page.title) : React.createElement("h2", null, page.title) : null, page.description ? React.createElement("p", null, page.description) : null);
}

var FormControl = /*#__PURE__*/React.memo(function FormControl(_ref2) {
  ...
  return React.createElement("div", null, label && React.createElement("label", {
    htmlFor: labelFor
  }, label), description && React.createElement("div", null, description), children, error && React.createElement("div", null, error));
});
...
var getElementsFromForm = function getElementsFromForm(form) {
  ...
  return Object.values(form.schema.fields).filter(function (i) {
    return i.type !== types.FormElementType.GROUP && i.type !== types.FormElementType.PAGE && i.type !== types.FormElementType.CHOICE;
  });
};
/**
...
function getValidationSchema(inputElements) {
  return Yup.object(inputElements && inputElements.reduce(function (prev, curr) {
    var validation;
    ...
    if (curr.type === types.FormElementType.CHECKBOX) {
      validation = Yup.array().of(Yup.string());
    } else if (curr.type === types.FormElementType.RADIO && curr.children && curr.children.length > 0) {
    ...
    } else if (curr.type === types.FormElementType.SHORT_TEXT || curr.type === types.FormElementType.LONG_TEXT) {
      validation = Yup.string();

      if (curr.maxLength) {
        validation = validation.max(curr.maxLength, "Must be less than " + curr.maxLength);
      }
    } else if (curr.type === types.FormElementType.EMAIL) {
    ...
    } else if (curr.type === types.FormElementType.PHONE_NUMBER) {
      validation = Yup.string();

      if (curr.maxLength) {
        validation = validation.max(curr.maxLength, "Must be less than " + curr.maxLength + " characters");
      }
    } else if (curr.type === types.FormElementType.DATE) {
      validation = Yup.date();
    } else if (curr.type === types.FormElementType.TIME) {
      validation = Yup.string();
    } else if (curr.type === types.FormElementType.URL) {
      validation = Yup.string().url('Invalid URL');

      if (curr.maxLength) {
        validation = validation.max(curr.maxLength, "Must be less than " + curr.maxLength + " characters");
      }
    } else if (curr.type === types.FormElementType.BOOLEAN) {
      validation = Yup.boolean();
    } else if (curr.type === types.FormElementType.NUMBER) {
...
var FormControlWrapper = /*#__PURE__*/React.memo(function (_ref7) {
  ...
  var _formik$getFieldMeta = formik$1.getFieldMeta(name),
...
function FormiumForm(_ref8) {
  ...
  var _React$useState = React.useState(_data),
  ...
  var setFieldProperty = React.useCallback(function (id, partial) {
    setForm(function (f) {
  ...
  return React.createElement(FormikWizard, Object.assign({
    ...
    return React.createElement(Page, {
    ...
    }, React.createElement(components.PageWrapper, null, React.createElement(components.Header, {
      form: form,
      page: page,
      pageIndex: pageIndex
    }), React.createElement(components.ElementsWrapper, null, page.children && page.children.map(function (element, index) {
      var id = element.id,
      ...
      return React.createElement(components.FieldWrapper, {
        key: id
      }, React.createElement(unstable_FormiumLogic, {
        form: form,
        element: element,
        setFieldProperty: setFieldProperty
      }), item.type === types.FormElementType.SHORT_TEXT ? React.createElement(FormControlWrapper, {
        required: !!required,
        label: item.title,
        labelFor: id,
        name: item.slug,
        ...
        id: id,
        type: "text"
      })) : item.type === types.FormElementType.EMAIL ? React.createElement(FormControlWrapper, {
        required: !!required,
        label: item.title,
        labelFor: id,
        name: item.slug,
        ...
        id: id,
        type: "email"
      })) : item.type === types.FormElementType.URL ? React.createElement(FormControlWrapper, {
        required: !!required,
        label: item.title,
        labelFor: id,
        name: item.slug,
        ...
        id: id,
        type: "url"
      })) : item.type === types.FormElementType.RADIO ? React.createElement(FormControlWrapper, {
        required: !!required,
        label: item.title,
        labelFor: id,
        name: item.slug,
        ...
        component: components.FormControl,
        disabled: false
      }, components.RadioGroup ? React.createElement(formik.Field, {
      ...
      }) : React.createElement(React.Fragment, null, children && children.length > 0 && children.map(function (c) {
      ...
      }))) : item.type === types.FormElementType.CHECKBOX ? React.createElement(React.Fragment, null, React.createElement(FormControlWrapper, {
        required: !!required,
        label: item.title,
        labelFor: id,
        name: item.slug,
      ...
      }))) : item.type === types.FormElementType.LONG_TEXT ? React.createElement(FormControlWrapper, {
        required: !!required,
        label: item.title,
        labelFor: id,
        name: item.slug,
...
function FormikWizard(_ref9) {
  ...
  var pageStorageKey = React.useMemo(function () {
    return getFormPageStorageKey(id);
  }, [id]);
  var stateStorageKey = React.useMemo(function () {
  ...
  var _React$useState2 = React.useState(initialValues || {}),
      values = _React$useState2[0],
      setValues = _React$useState2[1];

  var next = React.useCallback(function (newValues) {
    if (children) {
      setPage(function (p) {
        return Math.min(p + 1, React.Children.toArray(children).length - 1);
      });
      setValues(newValues);
    }
  }, [setPage, setValues, children]);
  var previous = React.useCallback(function () {
    setPage(function (p) {
      return Math.max(p - 1, 0);
    });
  }, [setPage]);
  var validationSchema = React.useCallback(function (v) {
  ...
  var handleSubmit = React.useCallback(function (currentValues, formikActions) {
    try {
      var _isLastPage = page === React.Children.count(children) - 1;
  ...
  var formik$1 = formik.useFormik(_extends({
    initialValues: values,
    onSubmit: handleSubmit,
    // enableReinitialize: true,
    validationSchema: validationSchema
  }, props));
  var setFormikState = formik$1.setFormikState,
      formikValues = formik$1.values,
      touched = formik$1.touched,
      errors = formik$1.errors,
      formikInitialValues = formik$1.initialValues;
  React.useEffect(function () {
    if (utils.canUseDOM()) {
  ...
  return React.createElement(formik.FormikContext.Provider, {
    value: formik$1
  }, React.createElement(formik.Form, null, React.createElement(React.Fragment, null, React.createElement(components.PageWrapper, null, activePage, React.createElement(components.FooterWrapper, null, page > 0 && React.createElement(components.PreviousButton, {
    type: "button",
    onClick: previous
  }, "Back"), !isLastPage && React.createElement(components.NextButton, {
    type: "submit",
    disabled: formik$1.isSubmitting
  }, "Next"), isLastPage && React.createElement(components.SubmitButton, {
    type: "submit",
    disabled: formik$1.isSubmitting
  }, "Submit"), debug ? React.createElement(React.Fragment, null, React.createElement("pre", {
    style: {
      marginTop: 12,
      fontSize: 11
    }
  }, JSON.stringify(formik$1, null, 2)), React.createElement("pre", {
    style: {
      fontSize: 11
    }
  }, JSON.stringify({
...
exports.FormiumForm = FormiumForm;
exports.defaultComponents = defaultComponents;
exports.getChoiceMap = getChoiceMap;
exports.getElementsFromForm = getElementsFromForm;
exports.getInitialValues = getInitialValues;
exports.getValidationSchema = getValidationSchema;
exports.unstable_FormiumLogic = unstable_FormiumLogic;
